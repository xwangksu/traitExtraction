...
finalFile = open(os.path.join(tPath, "GroundCover_op_final.csv"),'wt')
#------------------------------------------------------------------------
...
try:
    ...
    writer.writerow(('Plot_ID','Ground_Cover','H_Distance','H_Degree','V_Degree','Original_file','Plot_Center_Longitude',
    ...
    for i in range(len(pID)):
        ...
        v_dg = np.arctan2(diffDist, 10) * 180 / np.pi
        ts = imgFile[i].split("_")[1]
        writer.writerow((pID[i], gCover[i], '{0:.2f}'.format(diffDist),
