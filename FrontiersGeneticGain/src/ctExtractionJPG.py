...
finalFile = open(targetPath+"\\ctByPlot.csv", 'wt')
#------------------------------------------------------------------------
...
try:
    ...
    writer.writerow(('Plot_ID','Original_file','Median','Mode','NonZeroCount'))
    # imNum = 0
    for im in imList:
        # imNum += 1
        imObj = im.split("\\")
        ...
        imElement = imFileName.split("-20180")
...
#         rangeNum = pElement[1]
#         columNum = pElement[2]
        imageFileName = '20180'+imElement[numOfImElement-1]
...
#             plt.show()
            ...
            writer.writerow((pID,imageFileName,'{:.0f}'.format(medianFlat),'{:.0f}'.format(modeFlat),np.count_nonzero(imNpArray)))
finally:
    finalFile.close()