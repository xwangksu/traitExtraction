...
finalFile = open(targetPath+"\\REDEDGE_ByPlot_1.csv", 'wt')
#------------------------------------------------------------------------
...
try:
    ...
    writer.writerow(('Plot_ID','Original_file','Mean','Mode','NonZeroCount'))
    # imNum = 0
    for im in imList:
        # imNum += 1
        imObj = im.split("\\")
        ...
        imElement = imFileName.split("-20190")
...
#         rangeNum = pElement[1]
#         columNum = pElement[2]
        imageFileName = '20190'+imElement[numOfImElement-1]
...
#             plt.show()
            bn = int((imNzArray.max()-imNzArray.min())/0.01)
            hist, bin_edges = np.histogram(imNzArray, bins=bn, density=False)
            # print(imFlatNzArray)
            meanFlat = np.mean(imNzArray)/66536
            # print("mean_flat:", meanFlat)
            # medianFlat = np.median(imFlatNzArray)
            # print("median_flat:", medianFlat)
            modeFlat = float(bin_edges[np.argmax(hist)+1])/66536
            writer.writerow((plotID,imageFileName.replace("_RE.tif","_1.tif"),'{:.3f}'.format(meanFlat),'{:.3f}'.format(modeFlat),np.count_nonzero(imNpArray)))
