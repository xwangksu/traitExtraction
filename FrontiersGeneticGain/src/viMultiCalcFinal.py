...
finalFile = open(os.path.join(tPath, "viByPlot_final.csv"),'wt')
#------------------------------------------------------------------------
...
try:
    ...
    writer.writerow(('Plot_ID','Mean','Mode','Range','Column','NonZeroCount','H_Distance','H_Degree','V_Degree','Original_File','Plot_Center_Longitude',
...
#         pID[i] = pID[i].split("$")[0]
        ...
        writer.writerow((pID[i], '{0:.3f}'.format(np.double(vi[i])), '{0:.3f}'.format(np.double(vi2[i])), rangeNum[i], columnNum[i], nonZeroCount[i], '{0:.2f}'.format(diffDist),
