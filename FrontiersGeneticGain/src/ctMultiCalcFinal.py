...
#------------------------------------------------------------------------
dfKeep = pd.read_csv(sourceFile, usecols=[0,1,2,3,4,5,6], dtype=str)
pID = dfKeep['Plot_ID'].values.tolist()
vi = dfKeep['Mean'].values.tolist()
vi2 = dfKeep['Mode'].values.tolist()
imgFile = dfKeep['Original_file'].values.tolist()
rangeNum = dfKeep['Range'].values.tolist()
columnNum = dfKeep['Column'].values.tolist()
nonZeroCount = dfKeep['NonZeroCount'].values.tolist()

dfCoor = pd.read_csv(sourceFile, usecols=[7,8,9,10,11], dtype=np.double)
...
try:
    ...
    writer.writerow(('Plot_ID','Mean','Mode','Range','Column','NonZeroCount','H_Distance','V_Distance','H_Degree','V_Degree','Original_File','Plot_Center_Longitude',
...
#         pID[i] = pID[i].split("$")[0]
        ...
        ts = ts.split(".")[0]
        writer.writerow((pID[i], '{0:.1f}'.format(np.double(vi[i])), '{0:.1f}'.format(np.double(vi2[i])), rangeNum[i], columnNum[i], nonZeroCount[i], '{0:.2f}'.format(diffDist),
                         '{0:.2f}'.format(vDist), '{0:.2f}'.format(h_dg), '{0:.2f}'.format(v_dg), imgFile[i],'{0:.8f}'.format(plot_x[i]),'{0:.8f}'.format(plot_y[i]),
