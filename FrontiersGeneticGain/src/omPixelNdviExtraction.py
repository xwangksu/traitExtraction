...
finalFile = open(targetPath+"\\omPixelNDVIByPlot.csv", 'wt')
#------------------------------------------------------------------------
...
try:
    ...
    writer.writerow(('Plot_ID','Range_Row','Mean','Mode'))
    for im in imList:
        imObj = im.split("\\")
        numOfObj = len(imObj)
        imFileName = str(imObj[numOfObj-1])
        plotID = imFileName.split("$")[0]
        rr = "$"+ str(imFileName.split(".")[0]).split("$")[1] + "$"+ str(imFileName.split(".")[0]).split("$")[2]
        if plotID.find("-") != -1:
...
#             imFlatNzArray = imNpArray.flatten()
            ...
            writer.writerow((plotID,rr,'{:.3f}'.format(meanFlat),'{:.3f}'.format(modeFlat)))
finally:
    finalFile.close()