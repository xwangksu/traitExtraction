...
finalFile = open(os.path.join(targetPath, "GroundCoverRate_op.csv"),'wt')
try:
    ...
    for im in imList:
        ...
        ofName = str(im).split('$')[2]
        ofName = str(ofName).split('-')[1]
        originalFile = ofName.replace('_r.tif', '.tif')
        # Save output file
        writer.writerow((pid, '{0:.3f}'.format(groundCover), originalFile))
finally:
    finalFile.close() 